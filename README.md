...
• A book titled "OOP Concepts" written by "Will Jones". ISBN: 999-888, Price: 590 Baht.
Status: Borrowed. (3 copy)
• Member: "Somsak" (Member ID: M001) wants to borrow a book.
• The Action: Somsak borrows the book "Java Programming"
Instructions:
...
4. Update Logic: When checkOut() is called, the book's status must change to "Borrowed"
...
way (same title, same ISBN), do you think these two books are the same? And
what criteria would we use to determine the 'unique identity' of an object?
• Accountability & Responsibility: When designing the book borrowing process,
should the logic to verify a member's eligibility (e.g., checking if they have
reached the maximum limit of 3 books) reside within the Book class or the
